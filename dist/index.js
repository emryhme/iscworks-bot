...
app.post('/api/test-simulator/run-tests', auth_middleware_1.AuthMiddleware.authenticate, auth_middleware_1.AuthMiddleware.requireRole(['OWNER', 'ADMIN', 'MANAGER']), async (req, res) => {
    try {
        ...
        try {
            db_1.db.prepare("INSERT OR REPLACE INTO products (store_id, short_code, product_code, name, color, size, price, stock) VALUES (1, 'SIM1', 'SIM-A-100', 'Simülatör Ürünü A', 'Siyah', 'M', 100.0, 50)").run();
            db_1.db.prepare("INSERT OR REPLACE INTO inventory (store_id, product_code, stock) VALUES (1, 'SIM-A-100', 50)").run();
            db_1.db.prepare("INSERT OR REPLACE INTO products (store_id, short_code, product_code, name, color, size, price, stock) VALUES (3, 'SIM1', 'SIM-A-100', 'Simülatör Ürünü B (Gamma)', 'Kırmızı', 'M', 500.0, 99)").run();
            db_1.db.prepare("INSERT OR REPLACE INTO inventory (store_id, product_code, stock) VALUES (3, 'SIM-A-100', 99)").run();
            db_1.db.prepare("INSERT OR REPLACE INTO campaigns (id, store_id, title, description, code, active) VALUES (901, 1, 'Store A Özel İndirim', 'Store A Kampanyası', 'SIMKOD100', 1)").run();
            db_1.db.prepare("INSERT OR REPLACE INTO campaigns (id, store_id, title, description, code, active) VALUES (903, 3, 'Store B Özel İndirim', 'Store B Kampanyası', 'SIMKOD500', 1)").run();
        }
        catch { }
