...
class AdminCopilotService {
    ...
    static async processAdminCommand(userPrompt, storeId = 1) {
        ...
        const fiyatGuncelleTool = new tools_1.DynamicTool({
            ...
            func: async (inputStr) => {
                try {
                    ...
                    const res = db_1.db.prepare('UPDATE products SET price = ?, updated_at = CURRENT_TIMESTAMP WHERE store_id = ? AND (product_code = ? OR short_code = ?)').run(numPrice, storeId, productCode, productCode);
                    if (res.changes > 0) {
        ...
        const urunEkleTool = new tools_1.DynamicTool({
            ...
            func: async (inputStr) => {
                try {
                    ...
                    if (res.success) {
                        db_1.db.prepare('UPDATE products SET price = ? WHERE store_id = ? AND product_code = ?').run(numPrice, storeId, computedProductCode);
        ...
        const urunListeleSorgulaTool = new tools_1.DynamicTool({
            ...
            func: async (inputStr) => {
                try {
                    const parsed = typeof inputStr === 'object' ? inputStr : (inputStr ? JSON.parse(inputStr) : {});
                    const query = parsed.query || '';
                    const products = await stock_service_1.StockService.fetchAllSheetRows(storeId);
        ...
        while (response.tool_calls && response.tool_calls.length > 0 && count < 3) {
            ...
            for (const tc of response.tool_calls) {
                ...
                if (tc.name === 'STOK_GUNCELLE')
                    toolResult = await stokGuncelleTool.invoke(tc.args);
                else if (tc.name === 'FIYAT_GUNCELLE')
                    toolResult = await fiyatGuncelleTool.invoke(tc.args);
                else if (tc.name === 'SIPARIS_SORGULA')
                    toolResult = await siparisSorgulaTool.invoke(tc.args);
                else if (tc.name === 'URUN_EKLE')
                    toolResult = await urunEkleTool.invoke(tc.args);
                else if (tc.name === 'URUN_LISTELE_SORGULA')
                    toolResult = await urunListeleSorgulaTool.invoke(tc.args);
