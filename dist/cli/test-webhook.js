...
async function testHubChallenge() {
    ...
    try {
        ...
        const localRes = await axios_1.default.get(localUrl);
    ...
    try {
        ...
        const fixedRes = await axios_1.default.get(fixedUrl, {
            headers: {
...
async function simulateIncomingMessage() {
    ...
    rl.question('\nSeçiminiz (a-d): ', async (subChoice) => {
        ...
        if (cleanSub === 'b')
            msgText = 'KGMLW-M var mı?';
        else if (cleanSub === 'c')
            msgText = 'Emre İşcenkal 05428523712 Süleyman Mahallesi 1010 Sokak No 7 1 adet';
        else if (cleanSub === 'd') {
...
async function showDatabaseStats() {
    try {
        const ordersRes = await axios_1.default.get(`${BASE_URL}/api/orders`);
        const stocksRes = await axios_1.default.get(`${BASE_URL}/api/stocks`);
        console.log(`\n==================================================`);
        ...
        if (ordersRes.data.orders && ordersRes.data.orders.length > 0) {
            console.log('\n📋 Son 3 Sipariş:');
            ordersRes.data.orders.slice(0, 3).forEach((o, idx) => {
