...
async function runTestSuite() {
    ...
    await stock_service_1.StockService.addProduct({ storeId: 200, shortCode: 'ABC', productCode: 'ABC-M', name: 'Shirt B', size: 'M', stock: 50, price: 200 });
    const stockA = await stock_service_1.StockService.checkStock(100, 'ABC-M');
    const stockB = await stock_service_1.StockService.checkStock(200, 'ABC-M');
    ...
    await stock_service_1.StockService.deductStock(100, 'ABC-M', 5);
    const rowsA_after_deduct = await stock_service_1.StockService.fetchAllSheetRows(100);
    const rowsB_after_deduct = await stock_service_1.StockService.fetchAllSheetRows(200);
    ...
    const prodA_update = (await stock_service_1.StockService.fetchAllSheetRows(100)).find(r => r.productCode === 'ABC-M');
    const prodB_update = (await stock_service_1.StockService.fetchAllSheetRows(200)).find(r => r.productCode === 'ABC-M');
    ...
    await stock_service_1.StockService.deleteProduct(100, 'ABC-M');
    const stockA_del = await stock_service_1.StockService.checkStock(100, 'ABC-M');
    const stockB_del = await stock_service_1.StockService.checkStock(200, 'ABC-M');
    ...
    const crossSearch = await stock_service_1.StockService.checkStock(100, 'ABC-M');
    ...
    try {
        await stock_service_1.StockService.checkStock(undefined, 'ABC-M');
    }
    catch (e) {
        errCaughtStock = true;
    }
    ...
    try {
        inventory_service_1.InventoryService.getStock(null, 'ABC-M');
    }
    catch (e) {
        errCaughtInv = true;
    }
    ...
    await stock_service_1.StockService.addProduct({ storeId: 1, shortCode: 'DEF', productCode: 'DEF-L', name: 'Default Store Item', size: 'L', stock: 100 });
    const tenantAccess = inventory_service_1.InventoryService.getStock(200, 'DEF-L');
    ...
    await stock_service_1.StockService.addProduct({ storeId: 999, shortCode: 'TEST01', productCode: 'TEST-PROD-01', name: 'Test T-Shirt', size: 'M', stock: 1, price: 499, storeName: 'teststore' });
    const isAvailable1 = inventory_service_1.InventoryService.checkAvailability(999, 'TEST-PROD-01', 1);
    assert(isAvailable1 === true, '1 item request available when stock is 1');
    const isAvailable2 = inventory_service_1.InventoryService.checkAvailability(999, 'TEST-PROD-01', 2);
