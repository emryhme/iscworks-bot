...
async function runTestSuite() {
    ...
    const reqMasterAdmin = { auth: { userId: 1, storeId: 1, role: 'OWNER', email: 'tonystark@iscworks.com' } };
    auth_middleware_1.AuthMiddleware.requireMasterAdmin(reqMasterAdmin, mockResForbid, () => { masterAdminAllowed = true; });
    assert(masterAdminAllowed === true, 'Master Admin token (Store 1 OWNER) passes requireMasterAdmin middleware');
    console.log('\n1️⃣4️⃣ MASTER ADMIN TEST 2: Merchant Store 100 Access Rejection on Master Admin Route');
    let merchantBlockedOnMaster = false;
    const reqMerchant = { auth: { userId: 10, storeId: 100, role: 'OWNER', email: 'owner_a@iscworks.com' } };
    const mockResMasterForbid = { status: (code) => ({ json: (d) => { if (code === 403)
                merchantBlockedOnMaster = true; } }) };
    auth_middleware_1.AuthMiddleware.requireMasterAdmin(reqMerchant, mockResMasterForbid, () => { merchantBlockedOnMaster = false; });
    assert(merchantBlockedOnMaster === true, 'Merchant (Store 100 OWNER) blocked from Master Admin API with 403 Forbidden');
    console.log('\n1️⃣5️⃣ MASTER ADMIN TEST 3: Store Suspension & Activation Actions with Audit Logging');
    db_1.db.prepare("UPDATE stores SET status = 'suspended' WHERE id = 100").run();
    auth_middleware_1.AuthMiddleware.logAudit(1, 1, 'MASTER_ADMIN_SUSPEND_STORE', 'stores', '100', 'active', 'suspended');
    const store100Row = db_1.db.prepare("SELECT status FROM stores WHERE id = 100").get();
    const auditSuspend = db_1.db.prepare("SELECT * FROM audit_logs WHERE action = 'MASTER_ADMIN_SUSPEND_STORE' AND entity_id = '100'").get();
    ...
    db_1.db.prepare("UPDATE stores SET status = 'active' WHERE id = 100").run();
    auth_middleware_1.AuthMiddleware.logAudit(1, 1, 'MASTER_ADMIN_ACTIVATE_STORE', 'stores', '100', 'suspended', 'active');
    const store100Active = db_1.db.prepare("SELECT status FROM stores WHERE id = 100").get();
    ...
    const firstEvt = webhook_controller_1.WebhookController.isDuplicateEvent('evt_master_001', 100);
    const secondEvt = webhook_controller_1.WebhookController.isDuplicateEvent('evt_master_001', 100);
    ...
    db_1.db.prepare("UPDATE stores SET meta_page_id = 'page_100', instagram_account_id = 'ig_100' WHERE id = 100").run();
    db_1.db.prepare("UPDATE stores SET meta_page_id = 'page_200', instagram_account_id = 'ig_200' WHERE id = 200").run();
    const resStoreA = webhook_controller_1.WebhookController.resolveStoreByMetaId('page_100');
    ...
    const resStoreB = webhook_controller_1.WebhookController.resolveStoreByMetaId('page_200');
    ...
    const resolvedTarget = webhook_controller_1.WebhookController.resolveStoreByMetaId('page_100');
    ...
    const resolvedTargetQuery = webhook_controller_1.WebhookController.resolveStoreByMetaId('page_100');
    ...
    const convIdA = ai_service_1.AIService.getOrCreateConversation(100, 'sender_x');
    const convIdB = ai_service_1.AIService.getOrCreateConversation(200, 'sender_x');
    ...
    db_1.db.prepare("INSERT INTO customers (store_id, sender_id, name) VALUES (100, 'sender_x', 'Customer Alpha')").run();
    db_1.db.prepare("INSERT INTO customers (store_id, sender_id, name) VALUES (200, 'sender_x', 'Customer Beta')").run();
    const custA = db_1.db.prepare("SELECT name FROM customers WHERE store_id = 100 AND sender_id = 'sender_x'").get();
    const custB = db_1.db.prepare("SELECT name FROM customers WHERE store_id = 200 AND sender_id = 'sender_x'").get();
    ...
    const evtStoreARepeat = webhook_controller_1.WebhookController.isDuplicateEvent('evt_attack_001', 100);
    ...
    await stock_service_1.StockService.addProduct({ storeId: 100, shortCode: 'TST', productCode: 'TEST-STOCK', name: 'Test Product', size: 'M', stock: 10, price: 200 });
    const updateSuccess1 = await stock_service_1.StockService.updateStock(100, 'TEST-STOCK', 15);
    ...
    await stock_service_1.StockService.addProduct({ storeId: 200, shortCode: 'TST', productCode: 'TEST-STOCK', name: 'Test Product B', size: 'M', stock: 50, price: 200 });
    await stock_service_1.StockService.updateStock(100, 'TEST-STOCK', 30);
    const storeAStock = db_1.db.prepare("SELECT stock FROM products WHERE store_id = 100 AND product_code = 'TEST-STOCK'").get().stock;
    const storeBStock = db_1.db.prepare("SELECT stock FROM products WHERE store_id = 200 AND product_code = 'TEST-STOCK'").get().stock;
    ...
    await stock_service_1.StockService.addProduct({ storeId: 100, shortCode: 'ABC', productCode: 'ABC-M', name: 'Shirt A', size: 'M', stock: 10, price: 300 });
    await stock_service_1.StockService.addProduct({ storeId: 200, shortCode: 'ABC', productCode: 'ABC-M', name: 'Shirt B', size: 'M', stock: 100, price: 300 });
    await stock_service_1.StockService.updateStock(100, 'ABC-M', 20);
    const abcStoreA = db_1.db.prepare("SELECT stock FROM products WHERE store_id = 100 AND product_code = 'ABC-M'").get().stock;
    const abcStoreB = db_1.db.prepare("SELECT stock FROM products WHERE store_id = 200 AND product_code = 'ABC-M'").get().stock;
    ...
    const invalidNeg = await stock_service_1.StockService.updateStock(100, 'TEST-STOCK', -10);
    const invalidNaN = await stock_service_1.StockService.updateStock(100, 'TEST-STOCK', NaN);
    const currentStockAfterInvalid = db_1.db.prepare("SELECT stock FROM products WHERE store_id = 100 AND product_code = 'TEST-STOCK'").get().stock;
    ...
    const nonExistentResult = await stock_service_1.StockService.updateStock(100, 'NON-EXISTENT-CODE-999', 50);
